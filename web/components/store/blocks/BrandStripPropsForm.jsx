...
// Formulari de props del bloc "brand_strip" — camps 1:1 amb
// api/app/blocks/registry.py::BrandStripProps.
export default function BrandStripPropsForm({ props, onChange, onFieldChange }) {
  ...
  function addItem() {
    onChange({ ...props, items: [...items, { image_url: '', href: '' }] });
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          placeholder="També ens trobaràs a"
        ...
        {items.map((item, i) => (
          <div key={i} className="border border-zinc-200 rounded-xl p-3 space-y-2 relative">
            ...
            <div className="flex items-center gap-3">
              {item.image_url && (
                <img src={item.image_url} alt="" className="w-14 h-14 rounded-lg border border-zinc-200 object-contain p-1 shrink-0" />
              )}
              <label className="text-xs font-medium text-zinc-700 border border-zinc-200 rounded-xl px-3 py-2 cursor-pointer hover:bg-zinc-50 transition-colors">
                {uploadingIndex === i ? 'Pujant…' : item.image_url ? 'Canviar logo' : 'Pujar logo'}
