  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/verify-email?token=${token}`, {
      ...
      .then(async res => {
        ...
        login(access_token);
        setStatus('ok');
        setTimeout(() => router.replace('/compte'), 1200);
      })
      ...
      <div className="text-center max-w-sm">
        {status === 'loading' && (
          <>
            ...
            <p className="text-zinc-600">{t('confirmingYourEmail')}</p>
          </>
        )}
        {status === 'ok' && (
          <>
            <CheckCircle size={40} className="text-green-500 mx-auto mb-4" />
            <h1 className="font-serif italic text-2xl mb-2">{t('accountActivated')}</h1>
