...
export default function ProveidorsPage() {
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div>
        <h2 className="text-2xl font-bold text-zinc-900">Proveïdors</h2>
        <p className="text-sm text-zinc-500 mt-1">Compte corrent per proveïdor — saldo pendent i historial de factures.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-zinc-400 text-sm">Cap proveïdor donat d&apos;alta</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 text-xs text-zinc-500 border-b border-zinc-200">
              <tr>
                <th className="w-8 px-4 py-3" />
                <SortableTh label="Proveïdor" sortKey="nom" sort={sort} onSort={toggleSort} />
                <SortableTh label="Saldo pendent" sortKey="saldo" sort={sort} onSort={toggleSort} align="right" />
              </tr>
            </thead>
                  ...
                  <>
                    <tr key={p.id} onClick={() => toggleExpand(p)} className="hover:bg-zinc-50 cursor-pointer transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        {total === 0 ? (
                          <span className="text-zinc-400">Al dia</span>
                        ) : (
                    ...
                    {expanded === p.id && (
                      <tr key={`${p.id}-exp`}>
                        <td colSpan={3} className="px-6 py-3 bg-zinc-50/80 border-b border-zinc-100">
                          {!historial[p.id] ? (
                            <div className="text-xs text-zinc-400 py-2">Carregant historial...</div>
                          ) : historial[p.id].length === 0 ? (
                            <div className="text-xs text-zinc-400 py-2">Cap factura registrada</div>
                          ) : (
                            <table className="w-full text-xs">
                              <thead className="text-zinc-400">
                                <tr>
                                  <th className="text-left py-1 font-medium">Data</th>
                                  <th className="text-left py-1 font-medium">Concepte</th>
                                  <th className="text-right py-1 font-medium">Total</th>
                                  <th className="text-center py-1 font-medium">Estat</th>
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-zinc-100">
                                  ...
                                  <tr key={d.id}>
                                    ...
                                    <td className="py-1.5 text-center">
                                      {d.payment_status === 'pagat' ? (
                                        ...
                                        <span className="text-red-600 flex items-center justify-center gap-1"><AlertCircle size={11} /> Vençut</span>
                                      ) : (
                                        <span className="text-amber-600 flex items-center justify-center gap-1"><Clock size={11} /> Pendent</span>
                                      )}
                                    </td>
                                  </tr>
                                ))}
