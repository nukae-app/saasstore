  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-on-surface">{t('purchases.tab.suppliers')}</h2>
        <Button onClick={() => setShowModal(true)}>
          <MIcon name="add" size={16} /> {t('purchases.new_supplier')}
        </Button>
      </div>
        ...
        <form onSubmit={save} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.name', 'Nom *')}</label>
              <input value={form.name} onChange={e => f('name', e.target.value)} required
                className="w-full border border-outline-variant rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('common.type')}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.col.email', 'Email')}</label>
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.address', 'Adreça')}</label>
              <input value={form.address} onChange={e => f('address', e.target.value)}
                className="w-full border border-outline-variant rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.contact_person', 'Persona de contacte')}</label>
              <input value={form.contact} onChange={e => f('contact', e.target.value)}
                className="w-full border border-outline-variant rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.iban', 'Número de compte (IBAN)')}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.payment_method', 'Mètode de pagament')}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.payment_days', 'Dies de pagament')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface-variant mb-1">{t('purchases.supplier_modal.fixed_day', 'Dia fix del mes')}</label>
            ...
            <div className="col-span-2 flex items-center gap-2">
              ...
              <label htmlFor="proveedor-actiu" className="text-sm text-on-surface-variant">{t('purchases.supplier_modal.active_checkbox', 'Proveïdor actiu')}</label>
            </div>
          </div>
