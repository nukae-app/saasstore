  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-on-surface">{t('purchases.tab.particulars', 'Compres particulars')}</h2>
        <Button onClick={() => setShowModal(true)}>
          <MIcon name="add" size={16} /> {t('purchases.btn.individual_purchase', 'Compra particular')}
        </Button>
      </div>
        ...
        <form onSubmit={save} className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <label className="block text-sm font-medium text-on-surface-variant mb-1 mt-3">{t('purchases.individual_modal.individual_name', 'Nom particular')}</label>
            ...
            <div>
              ...
              <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} required
                className="w-full border border-outline-variant rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              <label className="block text-sm font-medium text-on-surface-variant mb-1 mt-3">{t('common.notes')}</label>
          ...
          <div className="border border-outline-variant rounded-xl p-4 space-y-3">
            <div className="text-sm font-semibold text-on-surface-variant">{t('purchases.modal.records', 'Discs comprats')}</div>
            {discogsEnabled && (
