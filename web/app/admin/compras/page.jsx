...
const DESPESA_ESTAT_LABEL = { pendent: 'Pendent', vencut: 'Vençuda', pagat: 'Pagada' };
...
const SOLICITUD_STATUS_LABEL = { oberta: 'Oberta', resolta: 'Resolta', cancelada: 'Cancel·lada' };
...
export default function ComprasPage() {
  ...
  const proveedoresColumns = useMemo(() => ({
    nombre: { sortValue: p => (p.nombre ?? '').toLowerCase() },
    tipo: { sortValue: p => p.tipo ?? '', filterValue: p => p.tipo },
    nif: { sortValue: p => p.nif ?? '' },
    email: { sortValue: p => p.email ?? '' },
    telefon: { sortValue: p => p.telefon ?? '' },
    actiu: { sortValue: p => p.actiu ? 'Actiu' : 'Inactiu', filterValue: p => p.actiu ? 'Actiu' : 'Inactiu' },
  }), []);
  const provQl = provQ.trim().toLowerCase();
  const proveedoresBuscats = useMemo(() => !provQl ? proveedores : proveedores.filter(p =>
    [p.nombre, p.nif, p.email, p.telefon, p.contacto].some(v => v && v.toLowerCase().includes(provQl))
  ), [proveedores, provQl]);
  const {
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
          ...
          {tab === 'comandas' ? (
            <>
              <button onClick={() => setShowFacturarModal(true)}
                ...
                <Receipt size={13} /> Facturar recepcions
              </button>
              <button onClick={() => setShowParticularModal(true)}
                className="flex items-center gap-1.5 text-sm border border-zinc-200 text-zinc-600 hover:bg-zinc-50 px-3 py-2 rounded-lg transition-colors">
                <Plus size={13} /> Compra particular
              </button>
            ...
            <>
              <button onClick={() => setShowRefillModal(true)}
                className="flex items-center gap-1.5 text-sm border border-zinc-200 text-zinc-600 hover:bg-zinc-50 px-3 py-2 rounded-lg transition-colors">
                <Sparkles size={13} /> Generar suggeriments
              </button>
              <Button onClick={() => setShowSolicitudModal(true)}>
                <Plus size={16} /> Nova sol·licitud
              </Button>
            </>
          ) : tab === 'proveedores' ? (
            <Button onClick={() => setShowModal(true)}>
      ...
      <div className="flex gap-1 bg-zinc-100 p-1 rounded-xl w-fit">
        {[['solicituds', 'Sol·licituds'], ['comandas', 'Comandes'], ['buscador', 'Cerca proveïdor'], ['resum', 'Resum'], ['proveedores', t('purchases.tab.suppliers')]].map(([key, label]) => (
      ...
      {tab === 'comandas' && (
        <div className="flex items-center gap-2 flex-wrap">
          <input value={filters.q} onChange={e => setFilters(f => ({ ...f, q: e.target.value }))}
            placeholder="Cerca per proveïdor, particular, núm. comanda o albarà..."
      ...
      {tab === 'proveedores' && (
        <>
          <input value={provQ} onChange={e => setProvQ(e.target.value)}
            placeholder="Cerca per nom, NIF, email, telèfon o contacte..."
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
            {proveedores.length === 0 ? (
              ...
              <div className="p-12 text-center text-zinc-400 text-sm">Cap proveïdor coincideix amb la cerca.</div>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-zinc-50 text-xs text-zinc-500 border-b border-zinc-200">
                  <tr>
                    <SortableTh label={t('common.name')} sortKey="nombre" sort={provSort} onSort={toggleProvSort} />
                    <SortableTh label={t('common.type')} sortKey="tipo" sort={provSort} onSort={toggleProvSort}
                      filterOptions={provDistinct.tipo} selected={provFilters.tipo} onFilterChange={setProvFilter} />
                    <SortableTh label="NIF" sortKey="nif" sort={provSort} onSort={toggleProvSort} />
                    <SortableTh label="Email" sortKey="email" sort={provSort} onSort={toggleProvSort} />
                    <SortableTh label="Telèfon" sortKey="telefon" sort={provSort} onSort={toggleProvSort} />
                    <SortableTh label="Estat" sortKey="actiu" sort={provSort} onSort={toggleProvSort} align="center"
                      filterOptions={provDistinct.actiu} selected={provFilters.actiu} onFilterChange={setProvFilter} />
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100">
                  {proveedoresFiltrats.map(p => (
                    <tr key={p.id} onClick={() => setEditingProveedor(p)} className="hover:bg-zinc-50 cursor-pointer transition-colors">
                      ...
                      <td className="px-5 py-3 text-zinc-500">{p.tipo ?? '—'}</td>
                      <td className="px-5 py-3 text-zinc-500">{p.nif ?? '—'}</td>
                      <td className="px-5 py-3 text-zinc-500">{p.email ?? '—'}</td>
                      <td className="px-5 py-3 text-zinc-500">{p.telefon ?? '—'}</td>
                      <td className="px-5 py-3 text-center">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${p.actiu ? 'bg-emerald-100 text-emerald-700' : 'bg-zinc-100 text-zinc-500'}`}>
...
function CompraParticularModal({ onClose, onSaved }) {
  ...
  function selectUser(u) {
    ...
    setUserResults([]);
    if (!nombreParticular) setNombreParticular(u.nombre || '');
  }
  ...
  async function resolveRelease({ discogsId, artista, titulo, sello, formato, anio, genero, estilos, pais, imagen_url, tracklist, credits }) {
    ...
    const rRes = await authFetch('/admin/releases', {
      method: 'POST',
      body: JSON.stringify({
        artista, titulo, sello: sello || null, formato: formato || null,
        anio: anio ? parseInt(anio) : null, genero: genero || null,
        estilos: estilos || null, pais: pais || null, imagen_url: imagen_url || null,
  ...
  async function save(e) {
    ...
    setSaving(true);
    const payload = {
      nombre_particular: nombreParticular || null,
      ...
      items: items.map(it => ({
        ...
        coste_adquisicion: parseFloat(it.coste_adquisicion),
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Compra particular</h3>
        ...
        <form onSubmit={save} className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">
                Usuari registrat <span className="text-zinc-400 font-normal">(opcional)</span>
              </label>
              {linkedUser ? (
                <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  ...
                  <span className="text-sm font-medium flex-1 truncate">{linkedUser.nombre || linkedUser.email}</span>
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-zinc-200 rounded-xl shadow-lg overflow-hidden">
                      {userResults.map(u => (
                        <button key={u.id} type="button" onClick={() => selectUser(u)}
                          ...
                          {u.nombre && <span className="text-zinc-400 ml-1 text-xs">{u.email}</span>}
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <label className="block text-sm font-medium text-zinc-700 mb-1 mt-3">Nom particular</label>
              <input value={nombreParticular} onChange={e => setNombreParticular(e.target.value)}
                placeholder="Nom i cognoms (si no és usuari registrat)"
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
            <div>
              ...
              <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} required
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
              <label className="block text-sm font-medium text-zinc-700 mb-1 mt-3">Notes</label>
          ...
          <div className="border border-zinc-200 rounded-xl p-4 space-y-3">
            <div className="text-sm font-semibold text-zinc-700">Discos comprats</div>
            <div className="relative">
              <input
                value={discogsQ}
                onChange={e => handleDiscogsQ(e.target.value)}
                placeholder="Cerca a Discogs..."
                disabled={resolving}
              ...
              />
              {searchingDiscogs && (
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-zinc-400">Cercant...</span>
              )}
              {discogsRes.length > 0 && (
            ...
            <button type="button" onClick={() => setManualMode(m => !m)}
              ...
              {manualMode ? 'Cancel·lar' : '+ Afegir disc a mà'}
            </button>
              ...
              <div className="p-3 bg-white rounded-xl border border-zinc-200 space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input type="number" value={manualForm.anio} onChange={e => setManualForm(f => ({ ...f, anio: e.target.value }))}
                    placeholder="Any" min="1900" max="2030"
                    className="border border-zinc-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-zinc-900" />
                </div>
                <Button type="button" size="sm" onClick={addManual}
                  disabled={resolving || !manualForm.artista.trim() || !manualForm.titulo.trim()}>
                  {resolving ? 'Creant...' : 'Afegir'}
                </Button>
              </div>
            )}

            {items.length === 0 && (
              <div className="text-sm text-zinc-400 text-center py-4">Encara no has afegit cap disc.</div>
            )}
              ...
              {items.map((it, idx) => (
                <div key={idx} className="p-3 bg-zinc-50 rounded-xl border border-zinc-200">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <div>
                      <label className="block text-xs text-zinc-500 mb-1">Preu de compra</label>
                    ...
                    <div>
                      <label className="block text-xs text-zinc-500 mb-1">Preu venda (catàleg)</label>
                    ...
                    {it.condicion === 'segona_ma' && <>
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1">Grading disc</label>
                      ...
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1">Grading funda</label>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving || items.length === 0 || (!nombreParticular && !linkedUser)}>
              {saving ? 'Desant...' : `Registrar compra (${items.length})`}
            </Button>
          </div>
        </form>
      </div>
...
const PROVEEDOR_TIPOS = [
  ['distribuidor', 'Distribuïdor'], ['proveidor_online', 'Proveïdor online'],
  ['subministrador', 'Subministrador'], ['professional', 'Professional'],
  ['transport', 'Transport'], ['particular', 'Particular'], ['altres', 'Altres'],
];
const PROVEEDOR_METODES_PAGAMENT = [
  ['transferencia', 'Transferència'], ['rebut_domiciliat', 'Rebut domiciliat'],
  ['targeta', 'Targeta'], ['efectiu', 'Efectiu'], ['paypal_altres', 'PayPal / altres'],
];

function emptyProveedorForm() {
  return {
    nombre: '', tipo: '', nif: '', email: '', telefon: '', direccion: '', contacto: '',
    actiu: true, iban_proveidor: '', metode_pagament: '', dies_pagament: '', dia_pagament_mes: '', notas: '',
  };
}
...
function ProveedorModal({ proveedor, onClose, onSaved }) {
  ...
  const [form, setForm] = useState(proveedor ? {
    nombre: proveedor.nombre ?? '', tipo: proveedor.tipo ?? '', nif: proveedor.nif ?? '',
    email: proveedor.email ?? '', telefon: proveedor.telefon ?? '', direccion: proveedor.direccion ?? '',
    contacto: proveedor.contacto ?? '', actiu: proveedor.actiu ?? true,
    iban_proveidor: proveedor.iban_proveidor ?? '', metode_pagament: proveedor.metode_pagament ?? '',
    dies_pagament: proveedor.dies_pagament ?? '', dia_pagament_mes: proveedor.dia_pagament_mes ?? '',
  ...
  async function save(e) {
    ...
    setError('');
    const payload = {
      nombre: form.nombre, tipo: form.tipo || null, nif: form.nif || null,
      email: form.email || null, telefon: form.telefon || null, direccion: form.direccion || null,
      contacto: form.contacto || null, actiu: form.actiu,
      iban_proveidor: form.iban_proveidor || null, metode_pagament: form.metode_pagament || null,
      dies_pagament: form.dies_pagament ? parseInt(form.dies_pagament, 10) : null,
      dia_pagament_mes: form.dia_pagament_mes ? parseInt(form.dia_pagament_mes, 10) : null,
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="font-bold text-zinc-900">{isEdit ? 'Editar proveïdor' : 'Nou proveïdor'}</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>
        <form onSubmit={save} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Tipus</label>
              <select value={form.tipo} onChange={e => f('tipo', e.target.value)}
                ...
                <option value="">—</option>
                {PROVEEDOR_TIPOS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.telefon} onChange={e => f('telefon', e.target.value)}
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-zinc-700 mb-1">Adreça</label>
              <input value={form.direccion} onChange={e => f('direccion', e.target.value)}
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-zinc-700 mb-1">Persona de contacte</label>
              <input value={form.contacto} onChange={e => f('contacto', e.target.value)}
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-zinc-700 mb-1">Número de compte (IBAN)</label>
              <input value={form.iban_proveidor} onChange={e => f('iban_proveidor', e.target.value)}
                placeholder="ESXX XXXX XXXX XXXX XXXX XXXX"
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Mètode de pagament</label>
              <select value={form.metode_pagament} onChange={e => f('metode_pagament', e.target.value)}
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 bg-white">
                <option value="">—</option>
                {PROVEEDOR_METODES_PAGAMENT.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-1">Dies de pagament</label>
                <input type="number" min="0" value={form.dies_pagament} onChange={e => f('dies_pagament', e.target.value)}
                  placeholder="30, 60..."
                  className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
              </div>
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-1">Dia fix del mes</label>
                <input type="number" min="1" max="31" value={form.dia_pagament_mes} onChange={e => f('dia_pagament_mes', e.target.value)}
                  className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
              </div>
            </div>
            <div className="col-span-2">
            ...
            <div className="col-span-2 flex items-center gap-2">
              <input type="checkbox" id="proveedor-actiu" checked={form.actiu} onChange={e => f('actiu', e.target.checked)}
                className="rounded border-zinc-300 text-amber-600 focus:ring-zinc-900" />
              <label htmlFor="proveedor-actiu" className="text-sm text-zinc-700">Proveïdor actiu</label>
            </div>
          </div>
          {error && <p className="text-red-500 text-sm">{error}</p>}
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="secondary" onClick={onClose}>Cancel·lar</Button>
            <Button type="submit" disabled={saving}>{saving ? 'Desant...' : isEdit ? 'Desar' : 'Crear'}</Button>
          </div>
        </form>
      </div>
    </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <form onSubmit={save} className="p-5 space-y-4">
          <div className="p-3 bg-zinc-50 rounded-xl text-sm">
            <span className="font-semibold text-zinc-900">{item.artista} — {item.titulo}</span>
            <span className="text-zinc-400 ml-2">{item.precio} €</span>
          </div>
          <div>
...
function ComandesTab({ comandas, comprasParticulars, comprasProveedor, loading, expanded, setExpanded, onRefresh, onRecepcio, onReturnItem, hasFilters }) {
  ...
  const rows = [
    ...comandas.map(c => ({ kind: 'comanda', id: c.id, fecha: c.fecha, raw: c })),
    ...comprasParticulars.map(c => ({ kind: 'particular', id: c.id, fecha: c.fecha, raw: c })),
  ].sort((a, b) => new Date(b.fecha) - new Date(a.fecha));

  const columns = useMemo(() => ({
    fecha: { sortValue: r => r.fecha ?? '' },
    origen: { sortValue: r => r.kind === 'comanda' ? 'Comanda' : 'Particular', filterValue: r => r.kind === 'comanda' ? 'Comanda' : 'Particular' },
    entitat: { sortValue: r => (r.kind === 'comanda' ? r.raw.proveedor_nombre : (r.raw.nombre_particular ?? r.raw.user_nom ?? '')).toLowerCase() },
    linies: { sortValue: r => (r.kind === 'comanda' ? r.raw.lineas?.length : r.raw.items?.length) ?? 0 },
    estat: {
      sortValue: r => r.kind === 'comanda' ? (COMANDA_STATUS_LABEL[r.raw.status] ?? r.raw.status) : 'Rebuda',
      filterValue: r => r.kind === 'comanda' ? (COMANDA_STATUS_LABEL[r.raw.status] ?? r.raw.status) : 'Rebuda',
    },
  }), []);
  ...
  async function downloadPdf(comanda) {
    ...
    a.href = url;
    a.download = `comanda_${comanda.num_comanda || comanda.id}.pdf`;
    a.click();
  ...
  async function downloadRecepcioPdf(compra) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `recepcio_${compra.num_albaran || compra.id}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  async function enviarPerEmail(comanda) {
    ...
    if (!r.ok) {
      const body = await r.json().catch(() => ({}));
      alert(body.detail || 'No s\'ha pogut enviar.');
    }
    setBusyId(null);
    onRefresh();
  }
  ...
  async function cancelar(comanda) {
    if (!confirm('Cancel·lar aquesta comanda?')) return;
  ...
  async function eliminar(comanda) {
    if (!confirm('Eliminar aquest esborrany de comanda?')) return;
    ...
    <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
      {loading ? (
        ...
        <div className="p-12 text-center text-zinc-400 text-sm">
          {hasFilters ? 'Cap resultat amb aquests filtres.' : 'Encara no hi ha comandes ni compres.'}
        </div>
      ) : (
        <table className="w-full text-sm">
          <thead className="bg-zinc-50 text-xs text-zinc-500 border-b border-zinc-200">
            <tr>
              ...
              <SortableTh label="Data" sortKey="fecha" sort={sort} onSort={toggleSort} />
              <SortableTh label="Origen" sortKey="origen" sort={sort} onSort={toggleSort}
                filterOptions={distinctValues.origen} selected={filters.origen} onFilterChange={setFilter} />
              <SortableTh label="Proveïdor / particular" sortKey="entitat" sort={sort} onSort={toggleSort} />
              <SortableTh label="Línies" sortKey="linies" sort={sort} onSort={toggleSort} align="center" />
              ...
              <th className="px-4 py-3 text-right font-medium">Accions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100">
      ...
      <tr onClick={() => setExpanded(expanded === c.id ? null : c.id)}
        ...
        <td className="px-4 py-3">
          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${COMANDA_STATUS_COLOR[c.status]}`}>
            {COMANDA_STATUS_LABEL[c.status] ?? c.status}
          </span>
        </td>
        <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-end gap-1.5">
            <button onClick={() => downloadPdf(c)} title="Descarregar PDF"
              className="p-1.5 text-zinc-400 hover:text-zinc-700 rounded-lg hover:bg-zinc-100">
              <FileText size={14} />
            </button>
            {c.status === 'esborrany' && (
              <button onClick={() => enviarPerEmail(c)} disabled={busyId === c.id + '_enviar'} title="Enviar per email"
            ...
            {c.status === 'esborrany' && (
              <button onClick={() => marcarEnviada(c)} disabled={busyId === c.id + '_marcar'} title="Marcar com a enviada (manual)"
            ...
            {(c.status === 'enviada' || c.status === 'rebuda_parcial') && (
              <button onClick={() => onRecepcio(c)} title="Registrar recepció"
            ...
            {c.status !== 'rebuda' && c.status !== 'cancelada' && (
              <button onClick={() => cancelar(c)} disabled={busyId === c.id + '_cancelar'} title="Cancel·lar"
                className="p-1.5 text-zinc-400 hover:text-red-500 rounded-lg hover:bg-red-50 disabled:opacity-50">
                <Ban size={14} />
              </button>
            )}
            {c.status === 'esborrany' && (
              <button onClick={() => eliminar(c)} disabled={busyId === c.id + '_eliminar'} title="Eliminar"
            ...
            <div className="space-y-1">
              {(c.lineas ?? []).map(l => (
                <div key={l.id} className="flex items-center gap-4 text-sm flex-wrap">
                  ...
                  <span className="text-zinc-500">{l.cantidad_rebuda}/{l.cantidad} rebudes</span>
                  {l.precio_unitario_estimado && (
                    <span className="text-zinc-400">Preu est.: {l.precio_unitario_estimado} €</span>
                  )}
                </div>
              ))}
            </div>
              ...
              <div className="mt-3 pt-3 border-t border-blue-100 space-y-1">
                ...
                {recepcions.map(r => (
                  <div key={r.id} className="flex items-center gap-3 text-sm flex-wrap">
                    ...
                    <span className="text-zinc-400">{r.num_albaran ? `Albarà ${r.num_albaran}` : 'Sense núm. albarà'}</span>
                    <span className="text-zinc-400">· {r.items?.length ?? 0} exemplars</span>
                    {r.despesa_estat ? (
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${DESPESA_ESTAT_COLOR[r.despesa_estat] ?? ''}`}>
                        Factura {DESPESA_ESTAT_LABEL[r.despesa_estat] ?? r.despesa_estat}
                      </span>
                    ) : (
                    ...
                    <button onClick={() => downloadRecepcioPdf(r)} title="Descarregar llista de recepció (per etiquetes de preu)"
  ...
  return (
    <>
      <tr onClick={() => setExpanded(expanded === c.id ? null : c.id)}
        ...
        <td className="px-4 py-3 font-medium">{c.nombre_particular ?? c.user_nom ?? '—'}</td>
        <td className="px-4 py-3 text-center">
          <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-zinc-100 text-zinc-700">
            {c.items?.length ?? 0}
          </span>
      ...
      {expanded === c.id && (
        <tr>
          <td colSpan={7} className="px-4 py-3 bg-amber-50/40 border-b border-amber-100">
            {(c.items?.length ?? 0) === 0
              ? <span className="text-sm text-zinc-400">Sense còpies.</span>
              : (
                <div className="space-y-1">
                  {c.items.map(it => (
                    <div key={it.item_id} className="flex items-center gap-4 text-sm flex-wrap">
                      <span className="font-semibold text-zinc-900">{it.artista} — {it.titulo}</span>
                      <span className="text-zinc-500">PVP: {it.precio} €</span>
                      {it.coste_adquisicion && (
                        <span className="text-zinc-400">Cost: {it.coste_adquisicion} €</span>
                      )}
                      {it.devuelto ? (
                        <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-zinc-100 text-zinc-500">
                          Retornat
...
function SolicitudsTab({ solicitudes, loading, proveedores, expanded, setExpanded, onRefresh }) {
  ...
  for (const l of lineasPendents) {
    const key = l.proveedor_sugerido_nombre || 'Sense proveïdor suggerit';
    if (!grups.has(key)) grups.set(key, []);
    grups.get(key).push(l);
  }
  const gruposOrdenados = [...grups.entries()].sort(([a], [b]) => {
    if (a === 'Sense proveïdor suggerit') return 1;
    if (b === 'Sense proveïdor suggerit') return -1;
  ...
  const solicitudsColumns = useMemo(() => ({
    ...
    origen: {
      sortValue: s => s.origen === 'refill_stock' ? 'Reposició per vendes' : 'Manual',
      filterValue: s => s.origen === 'refill_stock' ? 'Reposició per vendes' : 'Manual',
    },
    linies: { sortValue: s => s.lineas.length },
    estado: { sortValue: s => SOLICITUD_STATUS_LABEL[s.estado] ?? s.estado, filterValue: s => SOLICITUD_STATUS_LABEL[s.estado] ?? s.estado },
  }), []);
  const {
  ...
  async function eliminar(s) {
    if (!confirm('Eliminar aquesta sol·licitud?')) return;
  ...
  async function eliminarLinia(s, l) {
    if (!confirm(`Treure "${l.artista} — ${l.titulo}" d'aquesta sol·licitud?`)) return;
    setBusyId(l.id + '_eliminar_linia');
    const r = await authFetch(`/admin/solicitudes-compra/${s.id}/lineas/${l.id}`, { method: 'DELETE' });
    if (!r.ok) {
      const body = await r.json().catch(() => ({}));
      alert(body.detail || 'No s\'ha pogut eliminar el disc.');
    }
    setBusyId(null);
    onRefresh();
  }
    ...
    <div className="space-y-4">
      {lineasPendents.length > 0 && (
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-zinc-200">
            <div className="text-sm font-semibold text-zinc-900">Discos pendents de comanda</div>
            <div className="text-xs text-zinc-400">
              Marca els discos que vols agrupar en una mateixa comanda, encara que vinguin de sol·licituds diferents.
            </div>
          </div>
                  ...
                  {lineasGrup.map(l => (
                    <label key={l.id}
                      ...
                      <input type="checkbox" checked={seleccio.has(l.id)} disabled={!l.release_id}
                        ...
                        className="rounded border-zinc-300 text-amber-600 focus:ring-zinc-900" />
                      <span className="font-medium text-zinc-900">{l.artista} — {l.titulo}</span>
                      <span className="text-zinc-500">{l.cantidad}x</span>
                      {l.sello && <span className="text-zinc-400">{l.sello}</span>}
                      <span className="text-zinc-300 text-xs ml-auto">{new Date(l.solicitud_created_at).toLocaleDateString()}</span>
                      {!l.release_id && <span className="text-amber-600 text-xs">Cal donar d'alta el disc abans</span>}
                    </label>
                  ))}
                </div>
              );
          ...
          {seleccio.size > 0 && (
            <div className="px-5 py-3 bg-amber-50 border-t border-amber-100 flex items-center justify-between">
              <span className="text-sm font-medium text-amber-800">
                {seleccio.size} disc{seleccio.size !== 1 ? 's' : ''} seleccionat{seleccio.size !== 1 ? 's' : ''}
              </span>
              <div className="flex items-center gap-3">
                ...
                <Button size="sm" onClick={() => setResolvingLineas(lineasPendents.filter(l => seleccio.has(l.id)))}>
                  <ArrowRight size={13} /> Crear comanda
                </Button>
              </div>
            </div>
          )}
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-zinc-400 text-sm">Encara no hi ha cap sol·licitud de compra.</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 text-xs text-zinc-500 border-b border-zinc-200">
              <tr>
                <th className="w-8 px-4 py-3" />
                <SortableTh label="Data" sortKey="created_at" sort={solSort} onSort={toggleSolSort} />
                <SortableTh label="Origen" sortKey="origen" sort={solSort} onSort={toggleSolSort}
                  filterOptions={solDistinct.origen} selected={solFilters.origen} onFilterChange={setSolFilter} />
                <SortableTh label="Línies" sortKey="linies" sort={solSort} onSort={toggleSolSort} align="center" />
                <SortableTh label="Estat" sortKey="estado" sort={solSort} onSort={toggleSolSort}
                  ...
                  <Fragment key={s.id}>
                    <tr onClick={() => setExpanded(expanded === s.id ? null : s.id)}
                      ...
                      <td className="px-4 py-3 text-zinc-500">
                        {s.origen === 'refill_stock' ? 'Reposició per vendes' : 'Manual'}
                        {s.user_nom && <span className="text-zinc-400"> · {s.user_nom}</span>}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-zinc-100 text-zinc-700">
                          {s.lineas.length}{pendents > 0 && pendents < s.lineas.length ? ` (${pendents} pendents)` : ''}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${SOLICITUD_STATUS_COLOR[s.estado]}`}>
                          {SOLICITUD_STATUS_LABEL[s.estado] ?? s.estado}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1.5">
                          {s.estado === 'oberta' && pendents > 0 && (
                            <button onClick={() => setResolvingLineas(s.lineas.filter(l => !l.resuelta))} title="Resoldre cap a una comanda"
                              ...
                              <ArrowRight size={12} /> Resoldre
                            </button>
                          )}
                          {s.estado === 'oberta' && (
                            <button onClick={() => cancelar(s)} disabled={busyId === s.id + '_cancelar'} title="Cancel·lar"
                              className="p-1.5 text-zinc-400 hover:text-red-500 rounded-lg hover:bg-red-50 disabled:opacity-50">
                              <Ban size={14} />
                            </button>
                          )}
                          {pendents === s.lineas.length && (
                            <button onClick={() => eliminar(s)} disabled={busyId === s.id + '_eliminar'} title="Eliminar"
                              className="p-1.5 text-zinc-300 hover:text-red-500 rounded-lg hover:bg-red-50 disabled:opacity-50">
                              <Trash2 size={14} />
                            </button>
                          )}
                    ...
                    {expanded === s.id && (
                      <tr>
                        <td colSpan={6} className="px-4 py-3 bg-amber-50/40 border-b border-amber-100">
                                  ...
                                  <>
                                    ...
                                    {l.release_id && (
                                      <button onClick={() => setResolvingEstocLinea(l)}
                                        ...
                                        <PackageCheck size={11} /> Resoldre d'estoc
                                      </button>
                                    )}
                                    <button onClick={() => eliminarLinia(s, l)} disabled={busyId === l.id + '_eliminar_linia'}
                                      title="Treure aquest disc de la sol·licitud"
                          ...
                          {s.notas && <div className="mt-2 text-xs text-zinc-400">{s.notas}</div>}
                        </td>
                      </tr>
                    )}
                  </Fragment>
...
function ResoldreEstocModal({ linea, onClose, onSaved }) {
  ...
  async function resoldre(item) {
    ...
    try {
      ...
      if (r.ok) onSaved();
      else { const d = await r.json().catch(() => ({})); setError(d.detail || 'No s\'ha pogut resoldre.'); }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-sm w-full p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-zinc-900">Resoldre d'estoc</h2>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600"><X size={18} /></button>
        </div>
        <p className="text-sm text-zinc-500 mb-4">{linea.artista} — {linea.titulo}</p>
        <p className="text-xs text-zinc-400 mb-4">
          Tria l'exemplar que ja hi ha a estoc per tancar aquesta línia sense fer-ne una comanda a proveïdor.
        </p>
        {error && <p className="text-sm text-red-600 mb-3">{error}</p>}
        {items === null ? (
          <div className="animate-pulse bg-zinc-100 rounded-lg h-16" />
        ) : items.length === 0 ? (
          <p className="text-sm text-zinc-400 text-center py-6">Aquest disc no té cap exemplar disponible a estoc ara mateix.</p>
        ) : (
...
function NovaSolicitudModal({ proveedores, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Nova sol·licitud de compra</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>

        <form onSubmit={save} className="p-6 space-y-5">
          <p className="text-xs text-zinc-400">
            Afegeix els discos que vols comprar, encara sense triar proveïdor. Més endavant les
            resoldràs cap a una o diverses comandes des de la pestanya "Sol·licituds".
          </p>
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">Notes</label>
            <input value={notas} onChange={e => setNotas(e.target.value)}
          ...
          <div className="border border-zinc-200 rounded-xl p-4 space-y-3">
            <div className="text-sm font-semibold text-zinc-700">Discos volguts</div>
            <div className="relative">
              <input
                value={discogsQ}
                onChange={e => handleDiscogsQ(e.target.value)}
            ...
            <div className="space-y-2">
              {lineas.map((l, idx) => (
                <div key={idx} className="p-3 bg-zinc-50 rounded-xl border border-zinc-200">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <div>
                      <label className="block text-xs text-zinc-500 mb-1">Quantitat</label>
                    ...
                    <div>
                      <label className="block text-xs text-zinc-500 mb-1">Proveïdor suggerit</label>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving || lineas.length === 0}>
              {saving ? 'Desant...' : `Crear sol·licitud (${lineas.length})`}
            </Button>
          </div>
        </form>
      </div>
...
function ResoldreSolicitudModal({ lineas, proveedores, onClose, onSaved }) {
  ...
  async function save(e) {
    ...
    const payload = {
      ...
      lineas: pendents.filter(l => seleccio.has(l.id)).map(l => ({
        ...
        precio_unitario_estimado: preus[l.id] ? parseFloat(preus[l.id]) : null,
      })),
    };
    const r = await authFetch('/admin/solicitudes-compra/resolver', { method: 'POST', body: JSON.stringify(payload) });
    setSaving(false);
    if (r.ok) onSaved();
    else setError((await r.json().catch(() => ({}))).detail || 'No s\'ha pogut resoldre.');
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Crear comanda des de sol·licituds</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>

        <form onSubmit={save} className="p-6 space-y-5">
          <p className="text-xs text-zinc-400">
            Tria quines línies (poden venir de sol·licituds diferents) van a la mateixa comanda. Les que
            no seleccionis queden pendents per resoldre-les després (cap a un altre proveïdor, per exemple).
          </p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Proveïdor</label>
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-zinc-700 mb-1">Notes de la comanda</label>
              <input value={notas} onChange={e => setNotas(e.target.value)}
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
          </div>
            ...
            {pendents.map(l => (
              <label key={l.id} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-zinc-50">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-zinc-900 truncate">{l.artista} — {l.titulo}</div>
                  <div className="text-xs text-zinc-400">{l.cantidad}x{l.sello ? ` · ${l.sello}` : ''}</div>
                </div>
                <input type="number" step="0.01" min="0" placeholder="Preu est." value={preus[l.id] ?? ''}
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving || seleccio.size === 0 || !proveedorId}>
              {saving ? 'Creant comanda...' : `Crear comanda (${seleccio.size})`}
            </Button>
          </div>
        </form>
      </div>
...
function BuscadorProveidorTab({ onRefresh }) {
  ...
  function toggleCistella(l, proveedorId, proveedorNombre) {
    setCistella(prev => {
      ...
      return [...prev, {
        ...
        release_id: l.release_id ?? null, artista: l.artista, titulo: l.titulo,
        sello: l.sello, formato: l.formato, cantidad: 1,
      }];
    });
  }
  ...
  async function crearSolicitud(proveedorId, proveedorNombre) {
    ...
    try {
      const r = await authFetch('/admin/solicitudes-compra', {
        ...
        body: JSON.stringify({
          origen: 'manual',
          notas: `Generada des de Cerca proveïdor (${proveedorNombre})`,
          lineas: items.map(c => ({
            release_id: c.release_id || undefined,
            artista: c.release_id ? undefined : c.artista,
            titulo: c.release_id ? undefined : c.titulo,
            sello: c.release_id ? undefined : c.sello,
            formato: c.release_id ? undefined : c.formato,
            cantidad: c.cantidad,
      ...
      if (!r.ok) {
        const body = await r.json().catch(() => ({}));
        alert(body.detail || 'No s\'ha pogut crear la sol·licitud.');
        return;
      }
  ...
  const detalleColumns = useMemo(() => ({
    ...
    disc: { sortValue: r => (r.artista ? `${r.artista} ${r.titulo ?? ''}` : (r.notas ?? '')).toLowerCase() },
    sello: { sortValue: r => (r.sello ?? '').toLowerCase() },
    precio_coste: { sortValue: r => r.precio_coste != null ? parseFloat(r.precio_coste) : null },
  }), []);
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-3">
        <input
          ...
          placeholder="Cerca per artista, títol, segell... (opcional, per filtrar)"
          className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900"
        />
        <p className="text-xs text-zinc-400">
          Combina l'històric importat dels fulls de càlcul antics amb les comandes reals
          (enviada/rebuda) fetes des d'aquí — creix amb cada comanda nova. No indica estoc
          actual del proveïdor, només que se li ha comprat abans. Desplega un proveïdor per
          veure'n els discos.
        </p>
      </div>
            ...
            <div key={grup.proveedor_id} className="bg-blue-50/60 rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
              <div className="px-5 py-3 flex items-center justify-between border-b border-blue-100">
                <span className="text-sm font-semibold text-zinc-700">
                  {grup.proveedor_nombre} · {grup.items.length} {grup.items.length === 1 ? 'disc' : 'discos'}
                </span>
                <Button onClick={() => crearSolicitud(grup.proveedor_id, grup.proveedor_nombre)} disabled={creantProveidor === grup.proveedor_id}>
                  {creantProveidor === grup.proveedor_id ? 'Creant...' : 'Crear sol·licitud'}
                </Button>
              </div>
      ...
      {loading ? (
        <div className="text-sm text-zinc-400 text-center py-6">Carregant...</div>
      ) : (
        <>
          {resum.length === 0 ? (
            <div className="text-sm text-zinc-400 text-center py-6">Cap coincidència a l'històric.</div>
          ) : (
            <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 text-sm font-semibold text-zinc-700 border-b border-zinc-100">
                Proveïdors ({resum.length})
              </div>
                  ...
                  <div key={p.proveedor_id}>
                    ...
                    >
                      ...
                      <span className="text-zinc-500">
                        {p.count} {p.count === 1 ? 'compra' : 'compres'} · última {new Date(p.ultima_compra).toLocaleDateString()}
                      </span>
                    </button>
                    {expandedProv === p.proveedor_id && (
                      <div className="bg-zinc-50/60 border-t border-zinc-100 px-5 py-3">
                        {loadingExpanded ? (
                          <div className="text-sm text-zinc-400 py-2">Carregant...</div>
                        ) : (
                          <div className="space-y-1.5">
                            {expandedLineas.map(l => {
                              const addable = !!(l.release_id || (l.artista && l.titulo));
                              const selected = cistella.some(c => c.key === l.id);
                              return (
                                <div key={l.id} className="flex items-center gap-3 text-sm flex-wrap">
                                  <span className="text-zinc-400 w-20 shrink-0">{new Date(l.fecha).toLocaleDateString()}</span>
                                  <span className="font-medium text-zinc-900">
                                    {l.artista ? `${l.artista}${l.titulo ? ` — ${l.titulo}` : ''}` : (l.notas ?? '—')}
                                  </span>
                                  {l.ean && <span className="text-[10px] uppercase tracking-wide text-emerald-600 bg-emerald-50 rounded-full px-2 py-0.5">EAN</span>}
                                  {l.precio_coste != null && <span className="text-zinc-400 ml-auto">{l.precio_coste} €</span>}
                                  {addable && (
                                    <button
                                      onClick={() => toggleCistella(l, p.proveedor_id, p.proveedor_nombre)}
                                      title={selected ? 'Treure de la sol·licitud' : 'Afegir a sol·licitud'}
                                      className={`p-1 rounded-lg ${selected ? 'text-emerald-600 hover:bg-emerald-50' : 'text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100'} ${l.precio_coste == null ? 'ml-auto' : ''}`}
                                    >
          ...
          {detalle.length > 0 && (
            <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 text-sm font-semibold text-zinc-700 border-b border-zinc-100">
                Detall {q.trim().length >= 2 ? '(filtrat)' : '(més recents)'}
              </div>
              <table className="w-full text-sm">
                <thead className="bg-zinc-50 text-xs text-zinc-500 border-b border-zinc-200">
                  <tr>
                    <SortableTh label="Data" sortKey="fecha" sort={detalleSort} onSort={toggleDetalleSort} />
                    <SortableTh label="Proveïdor" sortKey="proveedor_nombre" sort={detalleSort} onSort={toggleDetalleSort} />
                    <SortableTh label="Disc" sortKey="disc" sort={detalleSort} onSort={toggleDetalleSort} />
                    <SortableTh label="Segell" sortKey="sello" sort={detalleSort} onSort={toggleDetalleSort} />
                    <SortableTh label="Cost" sortKey="precio_coste" sort={detalleSort} onSort={toggleDetalleSort} align="right" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100">
                  {detalleSorted.map(r => (
                    <tr key={r.id}>
                      <td className="px-4 py-3 text-zinc-500">{new Date(r.fecha).toLocaleDateString()}</td>
                      <td className="px-4 py-3 font-medium">{r.proveedor_nombre}</td>
                      <td className="px-4 py-3">{r.artista ? `${r.artista}${r.titulo ? ` — ${r.titulo}` : ''}` : (r.notas ?? '—')}</td>
                      <td className="px-4 py-3 text-zinc-400">{r.sello ?? '—'}</td>
                      <td className="px-4 py-3 text-right text-zinc-500">{r.precio_coste != null ? `${r.precio_coste} €` : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function RefillSugerenciesModal({ onClose, onSaved }) {
  ...
  async function save(e) {
    ...
    const payload = {
      origen: 'refill_stock',
      notas: `Generat automàticament (${new Date().toLocaleDateString()})`,
      lineas: candidats.filter(c => selected.has(c.release_id)).map(c => ({
        release_id: c.release_id,
        cantidad: parseInt(cantidades[c.release_id], 10) || 1,
    ...
    setSaving(false);
    if (r.ok) onSaved();
    else setError((await r.json().catch(() => ({}))).detail || 'No s\'ha pogut crear la sol·licitud.');
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Suggeriments de reposició</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>

        <div className="p-6 space-y-4">
          <p className="text-xs text-zinc-400">
            Discos amb estoc nou baix que es continuen venent (vendes dels últims 60 dies),
            amb menys de 21 dies d'estoc restant al ritme actual. No inclou discos amb una
            comanda ja oberta. La quantitat i el proveïdor són editables abans de crear la
            sol·licitud.
          </p>

          {loading ? (
            <div className="text-sm text-zinc-400 text-center py-8">Calculant...</div>
          ) : candidats.length === 0 ? (
            <div className="text-sm text-zinc-400 text-center py-8">
              Ara mateix no hi ha cap disc que compleixi els criteris de reposició.
            </div>
          ) : (
                  ...
                  <tr>
                    <th className="w-8 px-3 py-2">
                      <input type="checkbox"
                        ...
                        title={candidatsSorted.every(c => selected.has(c.release_id)) ? 'Desmarcar-ho tot' : 'Seleccionar-ho tot'}
                        className="rounded border-zinc-300 text-amber-600 focus:ring-zinc-900" />
                    </th>
                    <SortableTh label="Disc" sortKey="disc" sort={candSort} onSort={toggleCandSort} className="px-3 py-2" />
                    <SortableTh label="Estoc" sortKey="stock_actual" sort={candSort} onSort={toggleCandSort} align="center" className="px-3 py-2" />
                    <SortableTh label="Vendes 60d" sortKey="vendes_periode" sort={candSort} onSort={toggleCandSort} align="center" className="px-3 py-2" />
                    <SortableTh label="Dies estoc" sortKey="dies_estoc" sort={candSort} onSort={toggleCandSort} align="center" className="px-3 py-2" />
                    <SortableTh label="Marge" sortKey="marge_mitja" sort={candSort} onSort={toggleCandSort} align="right" className="px-3 py-2" />
                    <SortableTh label="Proveïdor" sortKey="proveedor_sugerido_nombre" sort={candSort} onSort={toggleCandSort} className="px-3 py-2"
                    ...
                    <th className="px-3 py-2 text-center font-medium">Quantitat</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100">
                      ...
                      <tr key={c.release_id} className={selected.has(c.release_id) ? '' : 'opacity-40'}>
                        ...
                        <td className="px-3 py-2.5">
                          ...
                          {c.devolucions_recents > 0 && (
                            <div className="text-[11px] text-red-500">⚠ {c.devolucions_recents} devolució{c.devolucions_recents > 1 ? 's' : ''} recent{c.devolucions_recents > 1 ? 's' : ''}</div>
                          )}
                        </td>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="button" onClick={save} disabled={saving || selected.size === 0}>
              {saving ? 'Creant...' : `Crear sol·licitud (${selected.size})`}
            </Button>
          </div>
        </div>
      </div>
...
function costCompra(c) {
  return (c.items ?? []).reduce((acc, it) => acc + parseFloat(it.coste_adquisicion || 0), 0);
}

function pendentQty(c) {
  return (c.lineas ?? []).reduce((acc, l) => acc + Math.max(0, l.cantidad - l.cantidad_rebuda), 0);
}
...
function ResumTab({ proveedores }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setComandesPendents(comandaLists.flat().sort((a, b) => new Date(a.fecha) - new Date(b.fecha)));
      if (comprasRes.ok) {
        const data = await comprasRes.json();
        setComprasPendents(data.sort((a, b) => new Date(a.fecha) - new Date(b.fecha)));
      }
      setLoading(false);
    })();
  }, []);
  ...
  if (loading) {
    return <div className="p-12 text-center text-zinc-400 text-sm">Carregant...</div>;
  }
  if (!stats) {
  ...
  const proveedorNom = Object.fromEntries((proveedores || []).map(p => [p.id, p.nombre]));

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatTile label="Despesa aquest mes" value={fmtEur(stats.total_mes)} />
        <StatTile label="Despesa aquest trimestre" value={fmtEur(stats.total_trimestre)} />
        <StatTile label="Despesa aquest any" value={fmtEur(stats.total_any)} />
        <StatTile label="Comandes pendents de rebre" value={stats.comandes_pendents}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-zinc-200">
            <div className="text-sm font-semibold text-zinc-700">Comandes pendents de rebre</div>
          </div>
          {comandesPendents.length === 0 ? (
            <div className="p-5 text-sm text-zinc-400">Cap comanda pendent de rebre.</div>
          ) : (
              ...
              {comandesPendents.map(c => (
                <div key={c.id} className="px-5 py-2.5 text-sm">
                  ...
                  <div className="text-xs text-zinc-400 mt-0.5">
                    ...
                    {c.num_comanda ? ` · ${c.num_comanda}` : ''} · {pendentQty(c)} discs pendents
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-zinc-200 flex items-center justify-between">
            <div className="text-sm font-semibold text-zinc-700">Recepcions pendents de facturar</div>
            <span className="text-xs text-zinc-400">{fmtEur(stats.sense_facturar_import)}</span>
          </div>
          {comprasPendents.length === 0 ? (
            <div className="p-5 text-sm text-zinc-400">Cap recepció pendent de facturar.</div>
          ) : (
            <div className="divide-y divide-zinc-100 max-h-72 overflow-y-auto">
                ...
                <div key={c.id} className="px-5 py-2.5 text-sm">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium text-zinc-900 truncate">
                      {c.proveedor_id ? (proveedorNom[c.proveedor_id] ?? '—') : (c.nombre_particular ?? '—')}
                    </span>
                    <span className="shrink-0 font-medium text-zinc-900">{fmtEur(costCompra(c))}</span>
                  </div>
                  <div className="text-xs text-zinc-400 mt-0.5">
                    {new Date(c.fecha).toLocaleDateString()}
                    {' · '}{c.num_albaran ? `Albarà ${c.num_albaran}` : 'Sense núm. albarà'}
                    {' · '}{c.items?.length ?? 0} exemplars
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5">
          <div className="text-sm font-semibold text-zinc-700 mb-3">Top proveïdors (últims 12 mesos)</div>
          {stats.top_proveidors.length === 0 ? (
            <div className="text-sm text-zinc-400">Encara no hi ha dades.</div>
          ) : (
...
function DespesaMensualChart({ serie }) {
  ...
  const data = serie.map(s => ({
    mes: s.mes,
    label: MESOS_CURT[parseInt(s.mes.split('-')[1], 10) - 1],
  ...
  return (
    <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="text-sm font-semibold text-zinc-700">Despesa mensual (últims 12 mesos)</div>
        <div className="flex items-center gap-4 text-xs text-zinc-500">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-blue-600" /> Proveïdor</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-amber-500" /> Particular</span>
        </div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-56" onMouseLeave={() => setHover(null)}>
        {ticks.map((t, i) => (
          <g key={i}>
            ...
            <text x={padL - 6} y={y(t)} textAnchor="end" dominantBaseline="middle" fill="#a1a1aa" fontSize={10}>
              {fmtTick(t)}
            </text>
          </g>
        ))}
      ...
      {hover !== null && (
        <div className="text-xs text-zinc-600 bg-zinc-50 rounded-lg px-3 py-2 inline-flex items-center gap-3">
          ...
          <span>Proveïdor: {fmtEur(data[hover].proveidor)}</span>
          <span>Particular: {fmtEur(data[hover].particular)}</span>
        </div>
      )}
    </div>
  );
...
function NovaComandaModal({ proveedores, onClose, onSaved }) {
  ...
  async function handleCsvFile(file) {
    ...
    try {
      ...
      if (!r.ok) { setCsvErrors([{ motiu: 'No s\'ha pogut llegir el fitxer.' }]); return; }
      const data = await r.json();
      setLineas(prev => [
        ...prev,
        ...data.lineas.map(l => ({
          release_id: l.release_id, artista: l.artista, titulo: l.titulo, existing: true,
          cantidad: l.cantidad, precio_unitario_estimado: l.precio_unitario_estimado ?? '',
        })),
      ]);
  ...
  async function save(e) {
    ...
    const payload = {
      ...
      lineas: lineas.map(l => ({
        release_id: l.release_id,
        cantidad: parseInt(l.cantidad, 10),
        precio_unitario_estimado: l.precio_unitario_estimado ? parseFloat(l.precio_unitario_estimado) : null,
      })),
    };
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Nova comanda</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={save} className="p-6 space-y-5">
          ...
          <p className="text-xs text-zinc-400">El número de comanda es genera automàticament en crear-la.</p>

          <div className="border border-zinc-200 rounded-xl p-4 space-y-3">
            <div className="text-sm font-semibold text-zinc-700">Discos demanats</div>
            <div className="relative">
              <input
                value={discogsQ}
                onChange={e => handleDiscogsQ(e.target.value)}
            ...
            {csvErrors.length > 0 && (
              <div className="border border-amber-200 bg-amber-50 rounded-lg p-2 text-xs text-amber-700 space-y-0.5">
                {csvErrors.map((e, i) => <div key={i}>Fila {e.fila ?? '?'}: {e.motiu}</div>)}
              </div>
            )}
              ...
              {lineas.map((l, idx) => (
                <div key={idx} className="p-3 bg-zinc-50 rounded-xl border border-zinc-200">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <div>
                      <label className="block text-xs text-zinc-500 mb-1">Preu unitari estimat</label>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving || lineas.length === 0 || !proveedorId}>
              {saving ? 'Desant...' : `Crear comanda (${lineas.length})`}
            </Button>
          </div>
        </form>
      </div>
...
function RecepcioModal({ comanda, onClose, onSaved }) {
  const pendents = (comanda.lineas ?? []).filter(l => l.cantidad_rebuda < l.cantidad);
  const [fecha, setFecha] = useState(new Date().toISOString().slice(0, 10));
  ...
  useEffect(() => {
    (async () => {
      ...
      const margeNou = marges.find(m => m.per_defecte_nou);
      const margeSegonaMa = marges.find(m => m.per_defecte_segona_ma);
      const ivaNou = tipusIva.find(t => t.per_defecte_nou);
      const ivaSegonaMa = tipusIva.find(t => t.per_defecte_segona_ma);
      setDefectesPreu({
        nou: margeNou && ivaNou ? { marge: parseFloat(margeNou.percentatge), iva: parseFloat(ivaNou.percentatge) } : null,
        segona_ma: margeSegonaMa && ivaSegonaMa ? { marge: parseFloat(margeSegonaMa.percentatge), iva: parseFloat(ivaSegonaMa.percentatge) } : null,
      });
    })();
  }, []);
  ...
  function addCopiaRebuda(linea) {
    setItemsPorLinea(prev => {
      ...
      const pendent = linea.cantidad - linea.cantidad_rebuda;
      if (unitatsLinea(actuales) >= pendent) return prev;
      return { ...prev, [linea.id]: [...actuales, { precio: linea.precio_unitario_estimado ?? '', condicion: 'nou', coste_adquisicion: '', estado_disco: '', estado_funda: '', cantidad: 1 }] };
    });
  }
  ...
  async function save(e) {
    ...
    for (const [lineaId, copias] of Object.entries(itemsPorLinea)) {
      for (const c of copias) {
        if (!c.precio) { setError('Falta el preu de venda en alguna còpia.'); return; }
        items.push({
          comanda_linea_id: lineaId,
          precio: parseFloat(c.precio),
          condicion: c.condicion,
          coste_adquisicion: c.coste_adquisicion ? parseFloat(c.coste_adquisicion) : null,
          estado_disco: c.condicion === 'nou' ? null : (c.estado_disco || null),
          estado_funda: c.condicion === 'nou' ? null : (c.estado_funda || null),
          cantidad: c.condicion === 'nou' ? (parseInt(c.cantidad, 10) || 1) : 1,
        });
      }
    }
    if (items.length === 0) { setError('Afegeix com a mínim una còpia rebuda.'); return; }

    setSaving(true);
    const r = await authFetch(`/admin/comandas/${comanda.id}/recepcio`, {
      method: 'POST',
      body: JSON.stringify({ fecha: new Date(fecha).toISOString(), num_albaran: numAlbaran || null, notas: notas || null, items }),
    });
    setSaving(false);
    if (!r.ok) {
      const body = await r.json().catch(() => ({}));
      setError(body.detail || 'No s\'ha pogut registrar la recepció.');
      return;
    }
    onSaved();
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Registrar recepció — {comanda.proveedor_nombre}</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={save} className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Data de l'albarà</label>
              <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} required
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">
                Núm. albarà <span className="text-zinc-400 font-normal">(opcional)</span>
              </label>
              <input value={numAlbaran} onChange={e => setNumAlbaran(e.target.value)}
                placeholder="Referència del proveïdor, no és la factura"
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            </div>
          </div>
          <p className="text-xs text-zinc-400 -mt-2">
            La factura es registra a part un cop arribi, des de "Facturar recepcions" — pot agrupar diverses recepcions.
          </p>
              ...
              return (
                <div key={linea.id} className="border border-zinc-200 rounded-xl p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-zinc-500">{unitatsLinea(copias)}/{pendent} pendents</span>
                  </div>
                  {copias.map((c, idx) => (
                    <div key={idx} className="flex flex-wrap items-end gap-3 p-2 bg-zinc-50 rounded-lg">
                      ...
                      {c.condicion === 'nou' && (
                        <div>
                          <label className="block text-xs text-zinc-500 mb-1">Unitats</label>
                          <input type="number" step="1" min="1" max={pendent - unitatsLinea(copias.filter((_, i) => i !== idx))}
                            ...
                            title="Aquesta entrada representa vàries unitats idèntiques (mateix cost i preu)"
                            className="w-16 border border-zinc-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-zinc-900" />
                        </div>
                      )}
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1">Cost {c.condicion === 'nou' ? '(per unitat)' : ''}</label>
                      ...
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1">Preu venda</label>
                        <div className="flex gap-1">
                          ...
                          <button type="button" onClick={() => calcularPreu(linea.id, idx, c)}
                            ...
                            title="Calcula el preu a partir del cost + marge i IVA per defecte"
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="text-[10px] text-zinc-400 mt-0.5 whitespace-nowrap">
                              Marge {desglos.importMarge.toFixed(2)}€ · IVA {desglos.importIva.toFixed(2)}€
                            </div>
                          );
                        })()}
                      </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving || totalCopias === 0}>
              {saving ? 'Desant...' : `Registrar recepció (${totalCopias})`}
            </Button>
          </div>
        </form>
      </div>
...
function FacturarRecepcionsModal({ proveedores, onClose, onSaved }) {
  ...
  async function save(e) {
    e.preventDefault();
    setError('');
    if (compraIds.length === 0) { setError('Selecciona com a mínim una recepció.'); return; }
    setSaving(true);
    ...
    if (!r.ok) {
      const body = await r.json().catch(() => ({}));
      setError(body.detail || 'No s\'ha pogut registrar la factura.');
      return;
    }
    onSaved();
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200">
          <h3 className="text-lg font-bold text-zinc-900">Facturar recepcions</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={save} className="p-6 space-y-5">
          ...
          {loadingCompras ? (
            <div className="text-sm text-zinc-400 text-center py-4">Carregant recepcions...</div>
          ) : proveedorId && compras.length === 0 ? (
            <div className="text-sm text-zinc-400 text-center py-4">Aquest proveïdor no té recepcions pendents de facturar.</div>
          ) : compras.length > 0 && (
            <div className="border border-zinc-200 rounded-xl divide-y divide-zinc-100">
              {compras.map(c => (
                <label key={c.id} className="flex items-center gap-3 px-4 py-2.5 text-sm cursor-pointer hover:bg-zinc-50">
                  <input type="checkbox" checked={!!selected[c.id]}
                    ...
                    className="rounded border-zinc-300 text-amber-600 focus:ring-zinc-900" />
                  <span className="text-zinc-500">{new Date(c.fecha).toLocaleDateString()}</span>
                  <span className="text-zinc-400">{c.num_albaran ? `Albarà ${c.num_albaran}` : 'Sense núm. albarà'}</span>
                  <span className="text-zinc-400">· {c.items?.length ?? 0} exemplars</span>
          ...
          {compras.length > 0 && (
            <>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-zinc-700 mb-1">Núm. factura</label>
              ...
              <div className="flex items-center justify-between text-sm bg-zinc-50 rounded-lg px-4 py-2.5">
                <span className="text-zinc-500">Total ({compraIds.length} recepcions)</span>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving || compraIds.length === 0}>
              {saving ? 'Desant...' : 'Registrar factura'}
            </Button>
          </div>
        </form>
      </div>
