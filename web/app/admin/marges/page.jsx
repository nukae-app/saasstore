...
export default function MargesPage() {
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      <div>
        <h2 className="text-2xl font-bold text-on-surface">{t('config.tab.margins', 'Marges')}</h2>
      </div>
      <MargesPanel />
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setEdit(null); setShowForm(true); }}>
          <MIcon name="add" size={16} /> {t('config.margins.new', 'Nou marge')}
        </Button>
      </div>
            ...
            <tbody className="divide-y divide-outline-variant">
              {marges.map(m => (
                <tr key={m.id} className="hover:bg-surface-container-high">
                  <td className="px-4 py-3 font-medium text-on-surface">{m.name}</td>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md">
        <div className="px-6 py-4 border-b border-outline-variant">
          <h3 className="text-lg font-bold text-on-surface">{isEdit ? t('config.margins.edit_title', 'Editar marge') : t('config.margins.new_title', 'Nou marge')}</h3>
        </div>
