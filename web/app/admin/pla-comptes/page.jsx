...
export default function PlaComptesPage() {
  ...
  const columns = useMemo(() => ({
    ...
    grup: { sortValue: c => c.group, filterValue: c => `Grup ${c.group}` },
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      <div>
        <h2 className="text-2xl font-bold text-zinc-900">Pla de comptes</h2>
        <p className="text-sm text-zinc-500 mt-1">
          Comptes del Pla General Comptable sembrats per aquest negoci. Es generen sols en donar-se d&apos;alta
          la forma jurídica a Configuració — no es creen ni editen des d&apos;aquí.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-zinc-400 text-sm">
            Encara no hi ha pla de comptes — fixa la forma jurídica a Configuració per sembrar-lo.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 text-xs text-zinc-500 border-b border-zinc-200">
                <tr>
                  <SortableTh label="Codi" sortKey="codi" sort={sort} onSort={toggleSort} />
                  <SortableTh label="Nom" sortKey="nom" sort={sort} onSort={toggleSort} />
                  <SortableTh label="Grup" sortKey="grup" sort={sort} onSort={toggleSort}
                    filterOptions={distinctValues.grup} selected={filters.grup} onFilterChange={setFilter} />
                  <SortableTh label="Tipus" sortKey="tipus" sort={sort} onSort={toggleSort} align="center"
              ...
              <tbody className="divide-y divide-zinc-100">
                {llista.map(c => (
                  <tr key={c.id} className={!c.active ? 'opacity-50' : ''}>
                    ...
                    <td className="px-4 py-3 text-zinc-500 text-xs">Grup {c.group}</td>
                    <td className="px-4 py-3 text-center"><TipusBadge tipus={c.account_type} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
