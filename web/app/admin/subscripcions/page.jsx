  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-on-surface">{t('subscriptions.title', 'Club del disc')}</h2>
      </div>
...
function CiclePanel() {
  ...
  return (
    <div className="space-y-5">
      {pendents.length > 0 && (
        <div className="bg-card rounded-2xl border border-outline-variant shadow-sm p-5 flex items-center justify-between">
          <div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-on-surface">{t('subscriptions.cycle.shipments_to_manage', 'Enviaments per gestionar')} ({enviaments.length})</h3>
        {enviaments.length > 0 && (
          <Button size="sm" onClick={confirmarTotes} disabled={confirmantTot}>
            {confirmantTot ? <MIcon name="progress_activity" size={14} className="animate-spin" /> : <MIcon name="check" size={14} />}
      ...
      {enviaments.length === 0 ? (
        ...
        <div className="space-y-4">
          {enviaments.map(env => {
            ...
            return (
              <div key={env.cobrament_id} className="bg-card rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
                  ...
                  <div className="flex gap-2">
                    {senseMatch > 0 && (
                      <button onClick={() => reintentarEnviament(env.cobrament_id)} disabled={reintentant === env.cobrament_id}
                        className="flex items-center gap-1.5 border border-outline-variant text-on-surface-variant px-3 py-1.5 rounded-lg text-sm hover:bg-surface-container-high transition-colors disabled:opacity-60">
                        {reintentant === env.cobrament_id ? <MIcon name="progress_activity" size={13} className="animate-spin" /> : <MIcon name="refresh" size={13} />}
                    ...
                    <Button size="sm" onClick={() => confirmarEnviament(env.cobrament_id)} disabled={trobats === 0}>
                      <MIcon name="check" size={14} /> {t('subscriptions.cycle.confirm_shipment', 'Confirmar enviament')}
                    </Button>
                  </div>
                </div>
                    ...
                    <div key={d.assignacio_id} className="px-5 py-3 flex items-center justify-between">
                      {d.item ? (
                        <div className="flex items-center gap-4 text-sm">
                          <span className="text-on-surface">{d.item.artista} — {d.item.titulo}</span>
            ...
            <tbody className="divide-y divide-outline-variant">
              {subscriptors.map(s => (
                <tr key={s.id} onClick={() => setObertId(s.id)}
                  ...
                  <td className="px-4 py-3 text-on-surface-variant">{s.proxima_facturacio}</td>
                  <td className="px-4 py-3 text-on-surface-variant">{s.ultim_disc_rebut || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
                ...
                {detall.generes_preferits.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {detall.generes_preferits.map(g => (
                      <span key={g} className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-primary text-white">
                        {g}
                      </span>
                    ))}
                  </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <label className="flex items-center gap-1.5 text-sm text-on-surface-variant bg-card border border-outline-variant rounded-xl px-3 py-2">
        ...
        <input type="number" placeholder={t('subscriptions.catalog.margin_min_ph', 'Marge mín. %')} value={margeMin} onChange={e => setMargeMin(e.target.value)}
          className="w-28 border border-outline-variant rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
      ...
      <div className="bg-surface-container-high border border-outline-variant rounded-xl p-4 flex flex-wrap items-center gap-3">
        <p className="text-sm text-on-surface-variant flex-1 min-w-[200px]">
        ...
        <label className="flex items-center gap-1.5 text-sm text-on-surface-variant">
          ...
          <input type="number" min="1" value={limitAuto} onChange={e => setLimitAuto(e.target.value)}
            className="w-20 border border-outline-variant rounded-lg px-2 py-1.5 text-sm" />
        </label>
        <Button size="sm" onClick={seleccionarAutomaticament} disabled={seleccionant}>
          {seleccionant ? <MIcon name="progress_activity" size={14} className="animate-spin" /> : <MIcon name="refresh" size={14} />}
      ...
      <div className="bg-card rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-outline-variant">
              {items.map(it => (
                <tr key={it.item_id} className="hover:bg-surface-container-high transition-colors">
                  <td className="px-4 py-3 text-on-surface">{it.artista} — {it.titulo}</td>
                  <td className="px-4 py-3">
                    {it.condicion === 'nou' ? (
                  ...
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => togglePool(it)}
                      className={`w-8 h-4 rounded-full transition-colors relative ${it.subscripcio_pool ? 'bg-green-500' : 'bg-muted'}`}>
                      <span className={`absolute top-0.5 w-3 h-3 rounded-full bg-card shadow transition-all ${it.subscripcio_pool ? 'left-4' : 'left-0.5'}`} />
                    </button>
                  </td>
                </tr>
              ))}
    ...
    <div className="bg-card rounded-2xl border border-outline-variant shadow-sm p-6 space-y-5 max-w-lg">
          ...
          {PERIODICITATS_CANDIDATES.map(m => (
            <button key={m} type="button" onClick={() => toggleList('periodicitats_mesos_disponibles', m)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                config.periodicitats_mesos_disponibles.includes(m) ? 'bg-primary text-white border-transparent' : 'border-outline-variant text-on-surface-variant hover:bg-surface-container-high'
              }`}>
          ...
          {QUANTITATS_CANDIDATES.map(q => (
            <button key={q} type="button" onClick={() => toggleList('quantitats_disponibles', q)}
              className={`w-9 h-9 rounded-full text-xs font-medium border transition-colors ${
                config.quantitats_disponibles.includes(q) ? 'bg-primary text-white border-transparent' : 'border-outline-variant text-on-surface-variant hover:bg-surface-container-high'
              }`}>
              {q}
            </button>
          ))}
      ...
      <div className="flex items-center gap-3 pt-1">
        <Button size="sm" onClick={save} disabled={saving}>
          <MIcon name="check" size={14} /> {saving ? t('common.saving') : t('common.save')}
        </Button>
