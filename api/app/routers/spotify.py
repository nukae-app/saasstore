...
def _redirect_uri() -> str:
    ...
    return f"{settings.frontend_url}/api/auth/spotify/callback"
...
async def spotify_init(request: Request, user: User = Depends(get_current_user)):
    ...
    if not settings.spotify_client_id:
        raise HTTPException(503, "Spotify no configurat al servidor")
    state = secrets.token_urlsafe(16)
    request.session[_SESSION_KEY] = {"uid": str(user.id), "state": state}
    url = svc.build_authorize_url(settings.spotify_client_id, _redirect_uri(), state)
    return {"url": url}
...
async def spotify_callback(
    ...
    try:
        token_data = await svc.exchange_code(
            code, _redirect_uri(), settings.spotify_client_id, settings.spotify_client_secret
        )
    except Exception:
        return RedirectResponse(f"{frontend}/compte?spotify=error")
...
async def spotify_library(
    ...
    try:
        access_token = await svc.ensure_fresh_token(
            conn, settings.spotify_client_id, settings.spotify_client_secret, db
        )
