...
- Un admin marcando manualmente como pagado un pedido con `payment_method="tienda"`
...
def finalize_payment(db: Session, order: Order) -> list[uuid.UUID]:
    ...
    discogs_token = get_discogs_token_if_enabled(db, order.tenant_id)
    rows = list(
        db.scalars(
            select(OrderItem)
    ...
    rows_segona = [r for r in rows_amb_item if r.condition != CondicionItem.nou]
    rows_nou = [r for r in rows_amb_item if r.condition == CondicionItem.nou]
    ...
    if hold_ids:
        ...
        for r in rows_nou:
            db.refresh(r.item)
            sync_stock_listing(db, r.item, discogs_token)
    ...
    if item_ids:
        for peticion in db.scalars(
            select(PeticionCliente).where(
                PeticionCliente.item_id.in_(item_ids), PeticionCliente.status == EstadoPeticionCliente.reservada,
            )
        ):
    ...
    for r in rows_amb_item:
        if r.item.codi_discogs:
            remove_item_from_discogs(discogs_token, r.item.codi_discogs)

    lang = order.language
    order_short = str(order.id)[:8]
    # `Tenant` no está filtrada por tenant (no tiene tenant_id propio), así
    # que esta búsqueda por `order.tenant_id` funciona sea cual sea el
    # tenant activo en la sesión en este momento.
    tenant = db.get(Tenant, order.tenant_id)
    comandes_url = f"{tenant_frontend_url(tenant)}/{lang}/compte/comandes"
    send_email(
        order.contact_email,
        translate(db, "email.order_confirmed.subject", lang, order_short=order_short, nom=tenant.nombre),
