...
from ..models import ConfiguracioBotiga, Tenant
from ..tenancy import tenant_frontend_url


def absolutize_upload_urls(html: str, tenant: Tenant) -> str:
    ...
    base = tenant_frontend_url(tenant)
    return re.sub(r'(src=["\'])(/uploads/)', rf"\1{base}\2", html)


def render_email_html(
    heading: str, body_html: str, tenant: Tenant, db, cta: tuple[str, str] | None = None,
) -> str:
    ...
    `db` hace falta para leer `ConfiguracioBotiga.address` de este tenant
    (nombre/logo salen de `Tenant`, la dirección solo vive en la config)."""
    config = db.scalar(select(ConfiguracioBotiga).where(ConfiguracioBotiga.tenant_id == tenant.id))
    adreca = (config.address if config else None) or ""
    logo_url = f"{tenant_frontend_url(tenant)}/logo.png"
    cta_html = ""
    if cta:
        label, url = cta
        cta_html = f"""
...
<div style="background:#f4f4f5;padding:32px 16px;font-family:-apple-system,Helvetica,Arial,sans-serif">
  <div style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden">
    <div style="background:#18181b;padding:24px;text-align:center">
      <img src="{logo_url}" alt="{tenant.nombre}" height="36" style="height:36px;width:auto">
    </div>
    ...
    <div style="padding:16px 28px;border-top:1px solid #e4e4e7;text-align:center">
      <p style="font-size:11px;color:#a1a1aa;margin:0">{tenant.nombre}{" · " + adreca if adreca else ""}</p>
    </div>
  </div>
</div>
"""
...
def send_email(
    ...
    `db` hace falta solo para leer `ConfiguracioBotiga.email_from` de este tenant."""
    s = get_settings()
    if not s.smtp_host:
        ...
        return
    config = db.scalar(select(ConfiguracioBotiga).where(ConfiguracioBotiga.tenant_id == tenant.id))
    email_from = (config.email_from if config else None) or f"{tenant.nombre} <no-reply@{tenant.domain}>"
    msg = EmailMessage()
    msg["From"] = email_from
    msg["To"] = to
    msg["Subject"] = subject
