...
def generate_comanda_pdf(comanda: Comanda, db: Session) -> bytes:
    ...
    titol = f"Comanda {comanda.order_number}" if comanda.order_number else "Comanda"
    pdf.cell(0, 8, titol, new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 5, f"Data: {comanda.date.strftime('%d/%m/%Y')}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 5, f"Proveïdor: {comanda.proveedor.name}", new_x="LMARGIN", new_y="NEXT")
    if comanda.notes:
        pdf.cell(0, 5, f"Notes: {comanda.notes}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(6)
    ...
    for linea in comanda.lineas:
        descripcio = f"{linea.release.artista} - {linea.release.title}"
        preu = linea.estimated_unit_price
        subtotal = (preu * linea.quantity) if preu is not None else None
        if subtotal is not None:
            total += subtotal
        pdf.cell(col_widths[0], 7, descripcio[:55], border=1)
        pdf.cell(col_widths[1], 7, str(linea.quantity), border=1, align="C")
