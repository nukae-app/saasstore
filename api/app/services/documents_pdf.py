"""Generació de PDF per als documents comercials: pressupostos, albarans i
factura de compra del Bloc B1 (sobre `Despesa`, sense taula pròpia), i
factura de venda del Bloc B2 (veure docs/PLAN_PARIDAD_HOLDED.md). Mateix
patró que comanda_pdf.py / recepcio_pdf.py — fpdf2, sense dependències
natives com WeasyPrint."""

from .pdf_common import new_pdf_with_header
from ..models import Albara, CondicionItem, Despesa, Factura, Pressupost
...
def generate_factura_pdf(factura: Factura, db) -> bytes:
    """Factura de venda (Bloc B2) — capa base del Reglament de Facturació,
    SENSE VeriFactu (sense hash, sense QR): ver docstring de models/documents.py."""
    pdf = new_pdf_with_header(db)

    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 8, f"Factura {factura.fiscal_year}/{factura.number:04d}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 5, f"Data: {factura.issue_date.strftime('%d/%m/%Y')}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 5, f"Client: {factura.client_name}", new_x="LMARGIN", new_y="NEXT")
    if factura.client_nif:
        pdf.cell(0, 5, f"NIF: {factura.client_nif}", new_x="LMARGIN", new_y="NEXT")
    if factura.client_address:
        adreca = factura.client_address
        linia = ", ".join(filter(None, [adreca.get("address_line1"), adreca.get("city"), adreca.get("postal_code")]))
        if linia:
            pdf.cell(0, 5, linia, new_x="LMARGIN", new_y="NEXT")
    if factura.status.value == "anullada":
        pdf.set_text_color(200, 0, 0)
        pdf.cell(0, 5, "FACTURA ANUL·LADA", new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(sum(col_widths[:4]), 7, "Base imposable", border=1, align="R")
    pdf.cell(col_widths[4], 7, f"{factura.base_total:.2f} EUR", border=1, align="R")
    pdf.ln()
    pdf.cell(sum(col_widths[:4]), 7, "IVA", border=1, align="R")
    pdf.cell(col_widths[4], 7, f"{factura.vat_total:.2f} EUR", border=1, align="R")
    pdf.ln()
    pdf.cell(sum(col_widths[:4]), 7, "Total", border=1, align="R")
    pdf.cell(col_widths[4], 7, f"{factura.total:.2f} EUR", border=1, align="R")
