...
def _seed_release_nou(db, cantidad=5, precio="20.00") -> tuple[Release, Item]:
    ...
    db.add(r)
    db.flush()
    item = Item(release_id=r.id, precio=Decimal(precio), condicion=CondicionItem.nou, cantidad=cantidad)
    db.add(item)
    db.commit()
    return r, item
...
def test_afegir_al_carret_amb_cantidad(db, client):
    ...
    resp = client.post("/cart/items", json={"item_id": str(item.id), "cantidad": 3})
    assert resp.status_code == 201
    body = resp.json()
    assert len(body["items"]) == 1
    assert body["items"][0]["cantidad"] == 3
    assert body["items"][0]["condicion"] == "nou"
    assert Decimal(body["total"]) == Decimal("60.00")

    # añadir 2 más: se suma a la misma línea
    resp2 = client.post("/cart/items", json={"item_id": str(item.id), "cantidad": 2})
    assert resp2.json()["items"][0]["cantidad"] == 5
...
def test_checkout_start_reserva_stockhold(db, client):
    _, item = _seed_release_nou(db, cantidad=5)
    client.post("/cart/items", json={"item_id": str(item.id), "cantidad": 3})

    resp = client.post("/checkout/start")
    assert resp.status_code == 200

    db.expire_all()
    assert db.get(Item, item.id).cantidad_reservada == 3
...
def test_checkout_start_falla_si_no_hay_suficiente_y_no_reserva_nada(db, client):
    ...
    item2 = Item(release_id=release.id, precio=Decimal("10.00"), condicion=CondicionItem.nou, cantidad=1)
    db.add(item2)
    db.commit()

    client.post("/cart/items", json={"item_id": str(item1.id), "cantidad": 3})
    ...
    client.post("/cart/items", json={"item_id": str(item2.id), "cantidad": 1})

    resp = client.post("/checkout/start")
    assert resp.status_code == 409

    db.expire_all()
    assert db.get(Item, item1.id).cantidad_reservada == 0  # revertido
...
def test_checkout_confirm_crea_orderitem_amb_cantidad_i_condicion(db, client):
    ...
    assert order_item.cantidad == 3
    assert order_item.condicion == CondicionItem.nou
    assert order_item.precio == Decimal("20.00")  # snapshot por unidad
    # el stock sigue reservado, no vendido, hasta que se pague de verdad
    assert db.get(Item, item.id).cantidad == 5
    assert db.get(Item, item.id).cantidad_reservada == 3
...
def test_pago_tienda_nou_vende_y_descuenta_cantidad(db, client):
    ...
    db.expire_all()
    assert db.get(Item, item.id).cantidad == 2
...
def test_cancelar_pedido_pagado_nou_repone_cantidad(db, client):
    ...
    db.expire_all()
    assert db.get(Item, item.id).cantidad == 5  # repuesto
