...
from app.models import Comanda, EstadoComanda, Item, Proveedor, Release, User
...
def _seed_proveedor(db, email="prov@example.com") -> Proveedor:
    p = Proveedor(nombre="DistroX", email=email)
    db.add(p)
    db.commit()
    return p
...
def test_numeracio_comanda_es_correlativa_per_any(db, client):
    ...
    release = _seed_release(db)
    payload = {
        "proveedor_id": str(prov.id), "fecha": "2026-01-15T10:00:00",
        "lineas": [{"release_id": str(release.id), "cantidad": 1}],
    }
    n1 = client.post("/admin/comandas", json=payload, headers=_auth(admin)).json()["num_comanda"]
    n2 = client.post("/admin/comandas", json=payload, headers=_auth(admin)).json()["num_comanda"]
    assert n1 == "2026-000001"
    assert n2 == "2026-000002"

    payload_altre_any = {**payload, "fecha": "2027-01-15T10:00:00"}
    n3 = client.post("/admin/comandas", json=payload_altre_any, headers=_auth(admin)).json()["num_comanda"]
...
def test_comanda_release_inexistent_falla(db, client):
    ...
    payload = {
        ...
        "lineas": [{"release_id": str(uuid.uuid4()), "cantidad": 1}],
    }
    resp = client.post("/admin/comandas", json=payload, headers=_auth(admin))
...
def test_pdf_comanda(db, client):
    ...
    payload = {
        "proveedor_id": str(prov.id), "fecha": "2026-06-01T10:00:00",
        "lineas": [{"release_id": str(release.id), "cantidad": 2, "precio_unitario_estimado": "10.00"}],
    }
...
def test_enviar_comanda_per_email(db, client, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(erp_module, "send_email", lambda **kw: calls.append(kw))

    resp = client.post(f"/admin/comandas/{comanda_id}/enviar", headers=_auth(admin))
    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "enviada"
    assert body["enviada_at"] is not None
    assert len(calls) == 1
...
def test_recepcio_total_crea_compra_i_items(db, client):
    ...
    recepcio_payload = {
        ...
        "num_albaran": "ALB-1",
        "items": [
            {"comanda_linea_id": linea_id, "precio": "20.00", "condicion": "segona_ma", "coste_adquisicion": "10.00"},
            {"comanda_linea_id": linea_id, "precio": "22.00", "condicion": "segona_ma", "coste_adquisicion": "10.00"},
        ],
    }
    ...
    assert comanda.lineas[0].cantidad_rebuda == 2
...
def test_recepcio_nou_agrega_en_una_sola_linia_amb_cost_mitja(db, client):
    ...
    payload = {
        "proveedor_id": str(prov.id), "fecha": "2026-06-01T10:00:00",
        "lineas": [{"release_id": str(release.id), "cantidad": 8, "precio_unitario_estimado": "10.00"}],
    }
    comanda_resp = client.post("/admin/comandas", json=payload, headers=_auth(admin)).json()
    comanda_id = comanda_resp["id"]
    linea_id = comanda_resp["lineas"][0]["id"]
    ...
    resp1 = client.post(
        ...
        json={
            "fecha": "2026-06-10T10:00:00",
            "items": [{"comanda_linea_id": linea_id, "precio": "20.00", "coste_adquisicion": "10.00", "cantidad": 5}],
        },
        headers=_auth(admin),
    )
    ...
    assert items[0].coste_adquisicion == Decimal("10.00")
    ...
    resp2 = client.post(
        ...
        json={
            ...
            "items": [{"comanda_linea_id": linea_id, "precio": "22.00", "coste_adquisicion": "14.00", "cantidad": 3}],
        },
        headers=_auth(admin),
    )
    ...
    assert item.coste_adquisicion == Decimal("11.50")  # (5*10 + 3*14) / 8
    assert item.precio == Decimal("22.00")  # el precio de venta se actualiza al de la última recepción
...
def test_recepcio_parcial_deixa_comanda_oberta(db, client):
    ...
    payload = {
        "proveedor_id": str(prov.id), "fecha": "2026-06-01T10:00:00",
        "lineas": [{"release_id": str(release.id), "cantidad": 3, "precio_unitario_estimado": "10.00"}],
    }
    comanda_resp = client.post("/admin/comandas", json=payload, headers=_auth(admin)).json()
    comanda_id = comanda_resp["id"]
    linea_id = comanda_resp["lineas"][0]["id"]
    ...
    recepcio_payload = {
        "fecha": "2026-06-10T10:00:00",
        "items": [{"comanda_linea_id": linea_id, "precio": "20.00"}],
    }
    resp = client.post(f"/admin/comandas/{comanda_id}/recepcio", json=recepcio_payload, headers=_auth(admin))
    assert resp.status_code == 201
    ...
    recepcio_payload_2 = {
        ...
        "items": [
            {"comanda_linea_id": linea_id, "precio": "20.00"},
            {"comanda_linea_id": linea_id, "precio": "20.00"},
        ],
    }
...
def test_recepcio_comanda_esborrany_falla(db, client):
    ...
    recepcio_payload = {"fecha": "2026-06-10T10:00:00", "items": [{"comanda_linea_id": linea_id, "precio": "20.00"}]}
    resp = client.post(f"/admin/comandas/{comanda_id}/recepcio", json=recepcio_payload, headers=_auth(admin))
    assert resp.status_code == 409
...
def test_resolver_csv_crea_releases_i_retorna_lineas(db, client, monkeypatch):
    ...
    def fake_get_release(discogs_release_id):
    ...
    monkeypatch.setattr(erp_module.discogs, "get_release", fake_get_release)
    ...
    assert by_id[str(r_existent.id)]["cantidad"] == 2
    assert by_id[str(r_existent.id)]["precio_unitario_estimado"] == 20.0

    nou_release = db.scalar(select(Release).where(Release.discogs_release_id == 222))
...
def test_resolver_csv_despres_es_pot_usar_per_crear_comanda(db, client, monkeypatch):
    ...
    payload = {
        "proveedor_id": str(prov.id), "fecha": "2026-06-01T10:00:00",
        "lineas": [{"release_id": linea["release_id"], "cantidad": linea["cantidad"], "precio_unitario_estimado": linea["precio_unitario_estimado"]}],
    }
    resp = client.post("/admin/comandas", json=payload, headers=_auth(admin))
    assert resp.status_code == 201
    assert resp.json()["lineas"][0]["cantidad"] == 3
