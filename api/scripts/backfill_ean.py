...
Ús:
  docker compose exec api python -m scripts.backfill_ean
  docker compose exec api python -m scripts.backfill_ean --force   # re-consulta tots
  docker compose exec api python -m scripts.backfill_ean --limit 100
"""
            ...
            for attempt in range(4):
                try:
                    ...
                    if i % 10 == 0 or i == len(releases):
                        ...
                        print(f"  [{i}/{len(releases)} {pct:.0f}%] {r.artista} — {r.titulo}: {r.ean or '(sense EAN a Discogs)'}", flush=True)
                    break
