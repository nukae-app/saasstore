...
Ús:
  docker compose exec api python -m scripts.backfill_formato --tenant recordstore
  docker compose exec api python -m scripts.backfill_formato --tenant recordstore --force   # re-consulta fins i tot els ja nets
  docker compose exec api python -m scripts.backfill_formato --tenant recordstore --limit 100
"""
...
def _run(db, tenant: Tenant, args) -> None:
    ...
    if not args.force:
        stmt = stmt.where(
            or_(RecordProduct.formato.is_(None), RecordProduct.formato.notin_(FORMATS_CANONICS))
        )
        ...
        for attempt in range(4):
            try:
                ...
                if i % 10 == 0 or i == len(releases):
                    ...
                    print(f"  [{i}/{len(releases)} {pct:.0f}%] {r.artista} — {r.title}: {r.formato or '(sense format a Discogs)'}", flush=True)
                break
